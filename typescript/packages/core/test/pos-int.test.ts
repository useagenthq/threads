...
// The vector pins the same boundary for Python, whose ints are unbounded.
test("the upper bound is the wire's", () => {
  expect(isPosInt(Number.MAX_SAFE_INTEGER)).toBe(true);
  expect(isPosInt(Number.MAX_SAFE_INTEGER + 1)).toBe(false);
});
