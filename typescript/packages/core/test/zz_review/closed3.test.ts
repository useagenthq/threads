...
test("a member runs after its lead closed", async () => {
  ...
  const seen: string[] = [];
  const base = scriptedModel({ responses: [call("x1", "nope_tool", {}), say("a"), say("b")] });
  const slow: Model = {
    ...base,
  ...
  const lead = agent({
    name: "lead",
    maxTurns: 1, model: scriptedModel({ responses: [start("c1", "researcher", "Go."), start("c2", "researcher", "x"), say("z")] }),
    team: [researcher],
  });
  const t0 = Date.now();
