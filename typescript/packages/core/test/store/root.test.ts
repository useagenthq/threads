...
import { fixture, rows as sqlAll, unwrap } from "./helpers";
...
test("two connections starting one thread get its first root", async () => {
  const path = join(mkdtempSync(join(tmpdir(), "root-")), "log.db");
  const a = await fixture("acme", openBunSqlite(path));
  const b = await fixture("acme", openBunSqlite(path));
  expect(unwrap(await a.store.rootOrCreate(THREAD, FIRST))).toBe(FIRST);
  expect(unwrap(await b.store.rootOrCreate(THREAD, SECOND))).toBe(FIRST);
  expect(unwrap(await b.store.mainBranch(THREAD))).toBe(FIRST);
  expect(
    await sqlAll(b.db, "SELECT branch_id FROM branches WHERE thread_id = ?", [
      THREAD,
    ]),
