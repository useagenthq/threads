...
type Opened = {
  ...
  readonly writer: Awaited<ReturnType<LogStore["acquire"]>>;
};
...
export async function open(
  ...
  const branch = await (typeof thread === "string"
  ...
  const writer = await log.acquire(branch.value, holder);
  ...
  if (create && writer.ok && writer.value.chain.fold.seq === 0) {
    const appended = await writer.value.append(first);
    if (!appended.ok)
