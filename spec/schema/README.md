...
**Before the build.** The events, rows and cases above are the contract; the Teams build implements them. Until it does, `validate_next` refuses with `unsupported_critical_event`, as for a critical event it doesn't know, every team event and every team form of an existing event (`thread_started` with `team` or a `team_member` parent, `user_input{source: team_task}`, a park or resume on `ask`, `wait` or `member`, `turn_completed{pin_*}`): a team log is never reduced as ordinary work. Both suites test it on the staged logs (`team-wire` tests). `woken` is not refused: both runtimes read it now (it opens a turn, rules 32 and 45 are checked), so the Phase 0 legacy wake is readable before its writer lands, and its cases are in the corpus. The team cases that need the other rules are staged (`../conformance/README.md`, "Staged cases"); `spec/tools/fixtures/ref_team.py` checks every case against a reference of rules 31-45 in `gen_fixtures.py --check`.
...
- **Nothing new after a barrier.** A run that finds `cancel_requested` in its open turn dispatches nothing new: calls that never began close `not_executed` (rule 8), and an in-doubt effect settles through recovery or parks (rule 11). A pending `spawn_agent` call whose child hasn't finished is not closed `not_executed`: the parent runs the child to its end (a child with no thread is recorded `cancelled` without being created; a child without a barrier in its open turn gets `cancel_requested{scope: tree}` first), records the child's one `agent_finished` and the call's result, and only then its own `cancelled` and `turn_completed{cancelled}` (`cancel-accepted-then-stopped`). `cancelled` and its `turn_completed{cancelled}` are one batch; recovery closes an open turn that already has a `cancelled` (an older writer's) with `turn_completed{cancelled}`, never `interrupted`. No `model_request` is recorded once the open turn has a barrier. The check is made where the request is recorded, in the same step (Python: also inside the request's own append), so a cancel from a `before_compact` hook, during a fallback's clearing or anywhere else before the send is honoured. A compaction side request that is not sent is answered by its `compaction_failed{summary, model_error}`, and the cancellation step ends the turn. No other end overtakes it: L4 preflight and L5 reactive compaction never end a turn with a pending cancel `context_exhausted`. A background child is cancelled the same way; its result arrives as `tool_result_late`.
...
- **Provider material.** Reasoning, redacted thinking and hosted tool items are replayed as recorded, so they are byte-exact. A response whose provider material holds a registered value stores none of it: `model_attempt_abandoned{provider_outcome: unknown, reason: provider_error}` and `turn_completed{error, code: secret_in_provider_output}` are appended in one batch, never retried. A compaction request ends its turn the same way; a requested compaction's `compaction_failed` is in that batch too. If a `cancel_requested` is already in the open turn, the batch leaves out `turn_completed`: the cancellation step then closes the turn as `cancelled`, so a cancel that lands during the attempt is never lost. An automatic compaction whose summary is refused this way records its `compaction_failed`, and with a cancel pending nothing more is sent: no turn request, no `context_exhausted`; the cancellation step is next.
