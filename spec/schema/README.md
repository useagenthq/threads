...
Every credential value the host resolves (a revealed Secret, or an adapter credential given as a string or defaulted to `secret("<ENV>")`) is **registered** in the host process. Nothing recorded holds a registered value: text is redacted, and bytes that must stay byte-exact are refused. A revealed Secret's label is its name; an adapter credential's is `<factory>.<option>` (`anthropic.apiKey` in TS, `anthropic.api_key` in Python); for equal values the smallest label wins.

- **Registration rules.** A value shorter than **8 characters** is refused with `ConfigError(invalid_config)`: "secret values must be at least 8 characters". So is a value equal to a literal of the exported event schema (an event type, an enum value or a field name). These rules are what make the exemption below safe: a registered value can never be a fixed wire field's value or a schema key.
- **Fixed wire fields are never redacted.** The writer leaves the line envelope (`type`, `type_version`, `seq`, `event_id`, `thread_id`, `branch_id`, `epoch`, `time`, `prev_hash`, `critical`) as built. It redacts every string of the line's `actor` and `data`, object keys included. A redacted key that meets another is numbered ` (2)`, ` (3)`, …; a numbered name that would hold a registered value is skipped for the next number.
...
- **Setup errors.** Every adapter `setup` and every MCP connect is wrapped: any failure, a `ConfigError` or any other exception, reaches `check()` (as an error value) and a run (as `ConfigError`) with its message redacted. An unexpected exception becomes `invalid_config` (`mcp_unreachable` for an MCP connect) naming the adapter or server.

Every write to the store, and the rule it follows:

| Write (TS / Python) | What is stored | Rule |
|---|---|---|
| Event lines (writer) | `actor` and `data` | redacted, fixed wire fields exempt |
...
| Spilled exec output (TS `redactingSink`; Python `Spill`) | stream | redacted as it streams |
...
| Local knowledge sources (`localKnowledge` ingest) | document bytes | byte-exact: ingest refused, `invalid_config` naming the path |
...
| Torn tail kept by import (`importThread` / `import_thread`) | log bytes | byte-exact: import refused, `secret_in_stored_bytes` |
