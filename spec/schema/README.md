...
- **Checks are atomic with the write.** A value registered between a check and the write it guards must not reach the store. TypeScript checks and writes synchronously. Python's store publishes on its own thread, so registration is paused while that thread re-checks and publishes: event rows (the content bytes again, `secret_in_stored_bytes`), every artifact put (`SecretInStoredBytesError`, nothing written; provider material maps it to `secret_in_provider_output`), a spill commit (the registry generation again) and an import.
