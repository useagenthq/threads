...
11. **Epoch fencing.** Epochs come from the branch lease. Acquiring a lease sets `epoch = max(lease epoch, max epoch in the resolved chain) + 1`, and every append by that holder, recovery included, uses it. A writer, gateway or adapter holding a lower epoch than the lease is stale and rejects with `stale_epoch` before dispatching anything. Fencing stops our own dispatch path. It never proves that an earlier dispatch did not happen.
    - **Recovering an open turn**. After the in-doubt items are settled, a turn with no `turn_completed` and nothing pending (no unsettled effect or call, no model request awaiting its response, nothing parked) is closed with `turn_completed{reason: interrupted}` (actor `recovery`), unless none of its input has been sent yet (no `model_request` follows the turn's last `user_input` or `steer`), or a `cancel_requested` in the turn has no `cancelled` yet. Then recovery appends nothing and the run continues the turn normally, or carries the cancel out (`recover-open-turn-cancel-requested-cancels`). This holds for every recovery, not only after a torn import (`recover-open-turn-answered-interrupted`, `recover-open-turn-unsent-continues`, `torn-tail-wrong-head-unterminated`).
12. **Branches, segments and fork.**
    ...
    - A `snapshot` fork records `knowledge_policy`: `pinned` (the API default) searches knowledge as of the fork snapshot's `knowledge_revision`; `current` searches the live corpus. A `repair` fork has neither a sandbox nor a knowledge policy. A snapshot on a thread with a knowledge binding always records `knowledge_revision`: when the provider's `revision` fails, no snapshot is taken at that turn end, so there is no fork point there rather than an unpinnable one.
...
- **Then one line per model-visible event, in log order:**
  ...
  | `message_received` | A user line whose text is `<message from="esc(NAME)" kind="KIND" untrusted="true">\nesc(TEXT)\n</message>`, from the receipt's envelope alone. Operator mail (`from: {operator}`) has `operator="true"` in place of `from="…"`, which no member name can produce. An `ask` adds `ask_id="esc(ID)"` after `kind`. TEXT is the body (a `{ref}` body is the verified artifact text), a notification's RFC 8785 JSON of its result with a `{ref}` output replaced by that text, or `bounced: <code>` for a bounce without a result. **Nothing** for a `reply`, a `cancel`, a `member_parked`, a bounce naming an ask, or a `member_settled` / `member_ended` for a settle monitor: those reach the model as the ask or wait call's `tool_result`, or not at all ([Teams](#teams); `render-team-message-lines`) |
...
**Background wakes (subagents).** When a background child's late result lands while its parent has no open turn, is not cancelled and has not ended (`member_ended`), the same append records `agent_finished`, `tool_result_late` (its `event_id` minted before the append) and `woken{causes}` naming it; several children of one run ending at one boundary share one `woken`. The woken turn belongs to the run that spawned those children: its `(principal, root_request)` is that run's turn opener's (a `user_input`, a turn-opening `message_received`, or an earlier `woken`), and its actor is `{host, principal}`. It renders nothing. A late result that lands while a turn is open is ordinary input to that turn, with no `woken`; one that lands after the parent ended is recorded without a `woken` (`legacy-late-result-after-lead-ended-no-wake`). A log written before `woken` existed is never woken retroactively (`subagent-background-notify` stays idle).

*Writer rule, a deviation from Gate 1 §2.7.3.* Gate 1 records every child ending at one boundary in one append with one `woken`. Children of different runs can't share a wake turn (one turn, one principal, one root request), so the writer records them by run, in spawn order: one run's results and its `woken` open a turn, and the next run's results stay unrecorded (their `pending_wakes` rows remain) until no turn is open, then get their own `woken` (`legacy-wakes-grouped-by-run`). `validate_next` checks only what one `woken` may name (rule 32); holding results back is the writer's job.

**Run completion (Gate 1 decision 27).** A run is named by its `user_input`, the run's request. It spans that turn and every later lead turn of the same request: a `woken` turn whose children the run spawned, and a turn opened by mail whose `provenance.root_request` is the run's `user_input`. It ends at the first point, in the lead's own log, where no lead turn is open, nothing is parked, and every run-owned helper has reported:
- a member is run-owned when its `member_started` is in the lead's log with `provenance.root_request` the run's request; it has reported when the lead's log holds a `message_received` of kind `member_settled` or `member_ended` naming its task monitor (`<lead branch_id>:<member_started event_id>:task`);
- a legacy child is run-owned when its `agent_spawned{mode: background}` is in a turn of the run; it has reported with its `agent_finished`.

`run()` then returns `completed`, whose `output` is the **last** answer of the run's turns (the last `model_response` of its last `turn_completed{end_turn}`); earlier answers stay in the timeline. A run turn that ends otherwise returns at once: `failed`, `cancelled`, `budget_exhausted` or `handed_off`. A lead parked on an approval or on a run-owned member (`{kind: member}`) returns `parked`. Members an operator started, or an earlier run, never extend the run, and a turn their mail opens after the end belongs to their own request (`run-operator-member-does-not-extend`). `run()`, `Thread` reads, host `subscribe` and replay all compute the end from these facts alone; the conformance projection is `run` (`../conformance/README.md`), with the reference in `spec/tools/fixtures/run_end.py` and the staged cases `run-*` and `legacy-run-*`.

**Tree walks with teams.** The tree cost walk (`Thread.cost({tree: true})`) and the tree usage walk behind it walk a lead's members as well as its subagents: every `team_members` row of `role: member` (never the lead's own row), every generation, model- or operator-started, recursively into nested teams, deduplicated by thread. A member with a branch is counted after its `thread_started.parent` is checked (`team_member`, the lead's thread and branch, and the lead's `member_started` or, for an operator start, the lead's `thread_started`); a mismatch is `log_corrupt`. A member in the starting window (a `starting` row, no branch, no task notification in its starter's log) counts **zero** and leaves `Cost.complete` true: it has made no model request, so nothing it spent is unknown. Any other row without a branch, including one whose starter already received its task notification, is `log_corrupt`. Cases: `team-tree-starting-member-pending`, `team-tree-missing-branch-after-notice-rejected`; the reference is `spec/tools/fixtures/tree_walk.py`.

**Names.** The team tools are pinned under the names `start`, `send`, `ask`, `reply`, `wait`, `monitor` and `cancel`; an agent with a `team` whose own tools use one of these names is refused at setup with `ConfigError("duplicate_name")`, naming the tool. The older `team_task_*` and `team_message` events and the `send_message` tool belong to today's subagent task board (Team tools above), not to these teams; the two coexist and share no rows.
...
**Read before the rest is built.** The events, rows and cases above are the contract, and the Teams build lands it in phases. Both runtimes read every team event: `validate_next` checks rules 31-45 on one log (43 is cross-log), a received mail opens a turn as above, and a `message_received` renders as its Render v1 line. The cases that need the team runner, the index, the tree walks, run completion or the `pending_wakes` projection stay staged (`../conformance/README.md`, "Staged cases"); both suites check that every staged log reads and reduces to its expected state. `spec/tools/fixtures/ref_team.py` checks every case against a reference of rules 31-45 in `gen_fixtures.py --check`.
...
A run started through `POST /v1/runs` survives a host crash. Once a host is ready (from its first tick, never inside `ready()`), it visits every branch named by a `run_receipts` row whose last event isn't `turn_completed`, and runs any turn there that is open and not parked, under the principal of that branch's last `user_input` and in the receipt's tenant. It records no new input: the loop's recovery settles what was in flight, so a begun effect is reconciled or parks and is never dispatched again. A second host on the same branch is refused by the lease; a branch the lease refuses is looked at again until its turn closes or parks, so a host restarted while a dead host's lease is still live resumes the run once that lease runs out. A subscriber that reconnects sees the resumed run's events and then its result under the original `run_id`. A `branch_busy` failure is never a run's result: another process holds the branch, and the log answers. A store outage (one the store raises as a store error: SQLite's busy, locked, I/O, full, can't-open, corrupt codes and the like, or a file-system error under the artifacts; never a SQL bug, and never judged by a code or class elsewhere) is retried, backing off from 1 s to 60 s while it lasts. A lookup that throws (a model's or an effect's) answers unknown inside the loop's recovery, never a host retry, unless it is a failed assertion: a bug fails the run. A recovery run that fails any other way is logged with its reason and not retried by that host; its turn stays open for a control, a new input or the next start. A `run_receipts` row that fails its schema is skipped and logged; the valid rows are still recovered.
...
Checked by readers and writers (`validate_next`) on top of the schema, except rules 14 and 15: they need artifacts, so import checks them in step 3, request verification (spec/conformance/README.md, `render`), and the adapter guard re-checks them before dispatch. This is the complete list of non-exportable rules: a hand-written check that isn't in this table is a CI error (AGENTS.md). Every rule has at least one conformance case. Rule 43's cases, the accepted cases of families not yet moved (`team-operator-start-and-wait`) and `woken-principal-mail-run` stay staged until the builds that run them ([Teams](#teams), "Read before the rest is built").
...
| 9 | `call_id`, `channel_delivery.item_key`, the `occurrence_id` of `schedule_fired` and `schedule_skipped` together (an occurrence is logged once, fired or skipped) and approval consumption per `challenge_id` are each unique along the resolved chain (ancestor segments through their fork points plus the branch), as in rule 28 | `invalid_transition` | `duplicate-call-id`, `schedule-occurrence-fired-then-skipped-rejected`, `schedule-occurrence-skipped-twice-rejected` |
...
| 31 | A `message_received`'s `mail_id` is its envelope's, and a mail is received, refused (`mail_refused`) or taken as a task (`user_input{team_task}`) at most once along the chain | `invalid_transition` | `team-receipt-mail-id-mismatch-rejected`, `team-receipt-twice-rejected`, `team-mail-refused-after-receipt-rejected`, `team-task-taken-twice-rejected` |
| 32 | `woken` needs no open turn and a thread that is not cancelled. Its `causes` are distinct and are every `tool_result_late` from the first cause to the `woken` (each event between is an `agent_finished` or one of them); a late result recorded earlier without a wake, by a writer from before `woken` or held for another run, is never named, each the late result of a background `spawn_agent` call, all spawned by one run: one `(principal, root_request)`, the spawning turn opener's, which becomes the woken turn's. A `tool_result_late` while a turn is open is ordinary input to that turn and needs no `woken` (Gate 1 §2.7.3), so every log written before `woken` existed stays valid | `invalid_transition` | `woken-with-open-turn-rejected`, `woken-cause-not-late-result-rejected`, `woken-cause-repeated-rejected`, `woken-mixed-runs-rejected` (one principal, two root requests); accepted: `legacy-wake-in-the-late-result-append`, `legacy-wake-after-pre-woken-late-result`, `legacy-wakes-grouped-by-run`, `legacy-late-result-in-another-runs-turn`, `render-reserved-control-events` |
| 33 | `team_opened` is only ever the resolved chain's first event (seq 1), and a chain that starts with it (a team log) takes only `operator_request`, `operator_refused`, `message_policy_decided`, `member_started`, `message_sent`, `message_received`, `mail_refused`, `ask_closed`, `wait_started`, `member_observed` and `wait_finished`, and a `fork{reason: repair}` (an inspection-only child) | `invalid_transition` | `team-log-foreign-event-rejected`, `team-opened-not-first-rejected`; accepted: `team-operator-start-and-wait`, `team-log-repair-fork` |
| 34 | A `task` is never a `message_received` (it arrives as `user_input{source: team_task}`), and mail of a kind that opens a turn (a `message`, an `ask`, a bounce naming no ask, a task or end monitor's notification: exactly the kinds that render) received while a turn is open shares the `(principal, root_request)` of the turn's opener, `root_request` compared whole (thread and event id), except that a member's turn opened by its task, whose root request is in the lead's log, compares the principal alone; other mail is exempt | `invalid_transition` | `team-task-as-message-rejected`, `team-mixed-provenance-turn-rejected`, `team-midturn-notice-other-request-rejected`, `team-receipt-root-thread-differs-rejected` |
...
| 37 | `member_ended` appears at most once, and after it the log opens no turn (no `user_input`, `woken` or turn-opening receipt) and appends no `member_idle` | `invalid_transition` | `member-ended-then-input-rejected`, `member-ended-then-user-input-rejected`, `member-ended-then-woken-rejected`; accepted: `legacy-late-result-after-lead-ended-no-wake` |
...
| 39 | A `wait_started` lists each member name once; `wait_finished` names a wait this log started and has not finished; `member_observed` names a monitor this log registered that has neither fired nor been observed | `invalid_transition` | `wait-finished-unknown-wait-rejected`, `wait-members-repeat-rejected`, `member-observed-after-fired-rejected` |
...
| 42 | In a team log, `operator_refused` and every event an operator request writes (a `message_sent` from `{operator: request_id}`, a `member_started`, whose `provenance.root_request` is this log's thread and an earlier `operator_request`'s event id, a `wait_started`, whose `wait_id` is `<this branch_id>:<request_id>`, a `message_policy_decided{request_id}`) follow that request's `operator_request`; a `message_policy_decided{call_id}` names a pending `tool_call` (one with no `tool_result` yet, along the resolved chain) | `invalid_transition` | `operator-refused-without-request-rejected`, `operator-start-without-request-rejected`, `operator-wait-without-request-rejected`, `policy-decision-after-result-rejected` |
...
| 45 | Bindings within one log: a `message_received`'s actor principal is its envelope's `provenance.principal`; an `operator_request`'s actor principal, `principal` and `provenance.principal` are one principal, and its `provenance.root_request` is the event itself (this thread and event id); a `member_started` in a lead's log names the event itself as `parent` (thread, branch and event id), and in a team log names the thread `team_opened.lead_thread_id`; a `woken`'s actor principal is the principal of the run that spawned each of its causes' children: the actor principal of the turn opener (a `user_input`, a turn-opening `message_received`, or an earlier `woken`) of the turn holding that child's `agent_spawned` | `invalid_transition` | `receipt-actor-not-provenance-rejected`, `operator-request-root-not-itself-rejected`, `member-started-parent-not-itself-rejected`, `woken-principal-not-spawning-run-rejected`, `woken-principal-mail-run-rejected` (the latest `user_input` is Bob's, the spawning turn was opened by Alice's mail); accepted: `woken-principal-mail-run` |
