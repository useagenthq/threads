...
| 32 | `woken` needs no open turn and a thread that is not cancelled. Its `causes` are distinct and are exactly the `tool_result_late` events of its own append (every event from the first cause to the `woken` is an `agent_finished` or one of them), each the late result of a background `spawn_agent` call, all spawned by one run: one `(principal, root_request)`, the spawning turn opener's, which becomes the woken turn's. A `tool_result_late` while a turn is open is ordinary input to that turn and needs no `woken` (Gate 1 §2.7.3), so every log written before `woken` existed stays valid | `invalid_transition` | `woken-with-open-turn-rejected`, `woken-cause-not-late-result-rejected`, `woken-cause-repeated-rejected`, `woken-mixed-runs-rejected` (one principal, two root requests); accepted: `legacy-wake-in-the-late-result-append`, `legacy-wakes-grouped-by-run`, `legacy-late-result-in-another-runs-turn`, `render-reserved-control-events` |
