...
- **Checks are atomic with the write.** A value registered between a check and the write it guards must not reach the store. TypeScript checks and writes synchronously. Python's store publishes on its own thread, so registration is paused while that thread re-checks and publishes: event rows and a fork's `fork` event (the content bytes again, `secret_in_stored_bytes`; nothing was written, so the writer goes on), every artifact put (`SecretInStoredBytesError`, nothing written; provider material maps it to `secret_in_provider_output`, a knowledge provider to its `invalid` error), a spill commit (the registry generation again) and an import.
