...
**Sandbox image.** A remote sandbox image provides `/bin/sh`, `find`, `stat -c` (`%a`, `%s`), `sha256sum`, `od`, `cut`, `tr`, `sed`, `mkdir`, `env` (GNU coreutils or BusyBox). `lsp` also needs `python3` on the image `PATH`. A command's argv[0] is resolved on the provider `PATH`; the command then runs with exactly the tool env.
...
4. **Numbers.** Envelope fields and framework-authored integer fields are integers in `0 … 2^53−1`. Floats appear only in `JsonValue` positions (tool `input`, `model_params`, adapter `settings`), spelled per the spec. Fractions in policy are integer permille; prices are integer nano-units per token. Token counts are an integer or `null` (unknown, never zero).
