...
The boundary at MAX_SAFE_INTEGER is here because Python ints are unbounded and TypeScript numbers
are not: without it `is_pos_int` accepts 2**53 while `isPosInt` refuses it, and a timeout that far
out reaches the writer in Python alone.
...
ROWS: tuple[tuple[str, JsonValue, bool], ...] = (
    ...
    ("MAX_SAFE_INTEGER, the largest the wire holds exactly", MAX_SAFE_INTEGER, True),
    ("one past MAX_SAFE_INTEGER", MAX_SAFE_INTEGER + 1, False),
