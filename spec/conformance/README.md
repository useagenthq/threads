...
`staged/` holds cases for a spec whose Gate 2 is approved and whose build hasn't landed: today the Teams contract (`../schema/README.md`, "Teams", semantic rules 31-45). `gen_fixtures.py` writes and `--check`s them like the corpus, and both test suites check that every staged case validates against `case.schema.json` and every one of its lines passes the line schema. No runner executes them. The build moves each family from `_build_staged` to `FAMILIES` in `spec/tools/fixtures/__main__.py` in the PR that makes both runners pass it, and drops the pre-build refusal tests, so a staged case is never a skipped case.

**Planned with the build**, recorded here so the Phase 0 and Phase 1 proof lists are complete (Gate 1 §7): Phase 0's unknown-recipient refusal, handoff keeping its tool context, today's team parity and the Python run SSE bounded to its run; the legacy-wake schedules (two children at one boundary share one `woken`, a crash before the combined append, cancel after a late result, a parked child dropping its `pending_wakes` row, an index wipe); and Phase 1's two-process crash schedules, deadline and already-settled races, homogeneous batches, member parks, deletion, tree accounting and ancestry. They need a runtime or several processes, so they arrive as `team` cases or per-language tests with the build.
