...
`vectors/*.json` are small shared value vectors for rules that are not a whole case, such as manifest ordering (`vectors/manifest-order.json`). `tool-inputs.json`, `team-wire.json` (one team event line per entry, admitted or `invalid_line`), `handoff-transcripts.json` (a source log's event lines, a byte cap and the forwarded transcript), `agent-pins.json` (agent definitions and the `thread_started` and `config_hash` each pins; `../schema/README.md`, "The pinned config"), `team-ops.json` (the Teams store ops, shape in `team-ops.schema.json`; `../schema/README.md`, Teams, "Store ops") `dynamic.json` (a dynamic start's resolved fields and the exact block bytes; Teams, "Dynamic members"), `ui-thread-ids.json` (chat key to thread id, `../schema/ui/README.md`, "Chat key"), `ui-inputs.json` (UI request bodies, each accepted or refused with its code), `pos-int.json` (the accept/reject table for the `PosInt` check every public entry point makes on a positive-integer option, before anything is written; `../api.json`), `e2b-wire/cases.json` (each request both E2B adapters send to E2B's API and envd, a replayed answer and its outcome; the sources are cited in `spec/tools/fixtures/e2b_wire.py`) and the tree vectors `manifest-tar.json`, `archive-invalid.json` and `tree.json` (archives as base64, the trees they read as or the refusal, and tree artifact bytes; `../schema/README.md`, "Snapshot manifest") are generated by `gen_fixtures.py` like the cases. Each hand-written vector's expected value is checked by both test suites, and was computed independently by `spec/tools` and by the TypeScript implementation; change one only together with the rule it pins. `team-ops.json` is different: it is generated, and its expected outcomes and appended event types are authored in `spec/tools/fixtures/team_ops_*.py`, while its row changes are the reference executor's output (`ops_run.py` on `team_index.py`), not authored. Until the Teams build, both suites check only its shape and that every world event is a valid line; the lanes that build each op run its vectors.
...
cases/<name>/
  ...
  frames.jsonl     ui kind: the stream's frames, one canonical JSON line each
...
| `kind` | `reduce`, `render`, `recover`, `fork`, `stub`, `intake`, `host`, `policy`, `team`, `ui`, `security` or `parity` |
...
**`ui`** (`../schema/ui/README.md`)
1. Import the log into a fresh store as in `reduce` step 1; the `ui` receipts of `input.receipts` are rows of it.
2. Serve one connection of `input.protocol` for run `input.run_id` with the host's own mapping, closing and live code, and no route: the opening frame (`start`, or `RUN_STARTED` with `input.ids`, else the log's thread and run ids); with `input.after`, the cursor route's resumption; with `input.replay`, an AG-UI replay whose snapshot point is the connection's first head read; `threads.resume_conflict` frames for `input.resume` (all settled, so nothing is recorded; expiry is judged at `clock.now`). Without `input.live` the connection registers, then the whole log is committed and read once. With it, the steps run in order against a live hub: `register` registers the connection; `delta` reaches the hub; `commit` makes the log's head that seq; every step after `register` is followed by one read (the first is the connection's head read), so a delta can fall between registration and that read. Then the closing frames from the run's outcome at the log's end, and for the AI SDK `data: [DONE]`, recorded as the line `{"data":"[DONE]"}`, unless the connection closed on a live part that contradicts its commit.
3. Compare `frames.jsonl` byte for byte, one line `{id?, data}` per frame in canonical JSON. Validate every frame against the pinned protocol schema, and run the frames through the sequence rules: TypeScript through the stock clients (`processUIMessageStream` of `ai@7.0.113`; `@ag-ui/client@1.0.0`'s verifier and apply); Python through its port of their rules.
4. Compare `messages`: the stock client's messages after the frames, from an empty client (for an AI SDK cursor, from the client that received the canonical stream through the cursor). AI SDK: the assembled `UIMessage`s as `{id, role, parts}`, each part with only the keys `type`, `id`, `text`, `state`, `toolCallId`, `input`, `output`, `errorText`, `approval`, `preliminary` and `data` that are set. AG-UI: `agent.messages`, each with only `id`, `role`, `content`, `toolCallId` and `toolCalls` (each `{id, type, function: {name, arguments}}`) that are set.
...
`staged/` holds cases for a spec whose Gate 2 is approved and whose build hasn't landed. It is empty today: Phase 0 moved the legacy run and `pending_wakes` families (`legacy_run`, `legacy_wake_rows`), lane 21D `run_cases` and lane 21E the cancel cases (`team_cancel_rule`) to the corpus (`../schema/README.md`, "Teams", semantic rules 31-45). `gen_fixtures.py` writes and `--check`s them like the corpus, and both test suites check that every staged case validates against `case.schema.json`, every one of its lines passes the line schema, and every one of its logs reads (semantic rules and, when the case ships its artifacts, Render v1) and reduces to its expected state. The rest of each case (projections, tree walks, the index, cross-log rules) waits for the build that moves its family. `gen_fixtures.py --check` runs a reference `validate_next` for rules 31-45 (`spec/tools/fixtures/ref_team.py`) over every corpus and staged case: an accepted case breaks none of them, and a rejected case of one of them breaks it exactly at its expected seq, never earlier. The runs, tree walks and the index rebuild have their own references (`run_end.py`, `tree_walk.py`, `team_index.py`). Where an earlier event could plausibly break a rule first, a case also has an accepted twin whose log differs only in the breaking event (`woken-principal-mail-run`). Staged builders are listed by the phase whose build moves them (`STAGED_PHASE_0`: `legacy_run` and `legacy_wake_rows`, the `legacy-*` cases of the Phase 0 legacy wake; `STAGED_PHASE_1`: the team builders) in `spec/tools/fixtures/__main__.py`, and no builder mixes phases. The build moves each builder from its phase list to `FAMILIES` in the PR that makes both runners pass it, and drops the pre-build refusal tests, so a staged case is never a skipped case.

**Teams Phase 2** (lane 29) is staged apart, in `staged-phase-2/`, because both readers refuse its forms until its build (`../schema/README.md`, "Teams Phase 2"): both suites check that every case there validates against `case.schema.json`, every line passes the line schema, and every log reads either whole or as `unsupported_critical_event` at its first Phase 2 form, with at least one refused log per case. The reference validator checks them like the other staged cases (rules 50-55 in `spec/tools/fixtures/ref_host.py` and `ref_host_cross.py`), and the reference index (`team_index.py`) rebuilds their rows, callers' and tombstoned callers' included (`input.deleted`). Their `feed` projection (`input.feed`, `expected.feed`) is `team.events` over the rebuilt feed (epoch 1, offsets in `(branch_id, seq)` order): the items after a cursor, `epoch_restarted` first for an older epoch, and `invalid_cursor` for a later epoch or an offset past the head (`spec/tools/fixtures/host_feed.py`). The builders are `STAGED_PHASE_2` in `spec/tools/fixtures/__main__.py`; the build moves them into `FAMILIES` (and the cases into `cases/`) with the runners that pass them.

**Planned with the build**, recorded here so the Phase 0 and Phase 1 proof lists are complete (Gate 1 §7): Phase 0's today's team parity and the Python run SSE bounded to its run (per-language tests; the unknown-recipient refusal is `team-send-unknown-recipient-refused` and the handoff tool context is `vectors/handoff-transcripts.json`); the legacy-wake schedules (two children of one run at one boundary share one `woken`, children of two runs get one `woken` each in spawn order, a crash before the combined append, cancel after a late result, a parked child dropping its `pending_wakes` row, an index wipe); and Phase 1's two-process crash schedules, deadline and already-settled races, homogeneous batches, member parks, deletion, tree accounting and ancestry. The typed `agent()` overloads are type-tested in both languages with the build: `team` absent gives `Agent`, `team: []` and a non-empty team give `TeamAgent`, and `TeamAgent.run()` and `.stream().result` are `TeamRunResult` with `team`. They need a runtime or several processes, so they arrive as `team` cases or per-language tests with the build.
