...
`vectors/*.json` are small shared value vectors for rules that are not a whole case, such as manifest ordering (`vectors/manifest-order.json`). `tool-inputs.json`, `team-wire.json` (one team event line per entry, admitted or `invalid_line`) and `team-ops.json` (the Teams store ops, shape in `team-ops.schema.json`; `../schema/README.md`, Teams, "Store ops") are generated by `gen_fixtures.py` like the cases. Each vector's expected value is checked by both test suites, and was computed independently by `spec/tools` and by the TypeScript implementation. Unlike cases, a vector is small and hand-written; change one only together with the rule it pins.
...
| `states`, `index` | team: `reduce` of each log by label, and all eight team tables of `../schema/store.sql` rebuilt from the logs alone (`$defs/TeamIndex`), each sorted by its primary key, JSON columns parsed, mail's claim columns left out, `team_feed` as its `(branch_id, seq)` rows only (a rebuild starts a new epoch). `tree`: the lead's tree as cost and usage walk it, `{counted, pending}` member names (`../schema/README.md`, "Tree walks with teams"). A team error also names its `log` |
