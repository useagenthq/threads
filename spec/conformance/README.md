...
| `states`, `index` | team: `reduce` of each log by label, and all eight team tables of `../schema/store.sql` rebuilt from the logs alone (`$defs/TeamIndex`), each sorted by its primary key, JSON columns parsed, mail's claim columns left out, `team_feed` as its `(branch_id, seq)` rows only (a rebuild starts a new epoch). A team error also names its `log` |
...
`staged/` holds cases for a spec whose Gate 2 is approved and whose build hasn't landed: today the Teams contract (`../schema/README.md`, "Teams", semantic rules 31-45). `gen_fixtures.py` writes and `--check`s them like the corpus, and both test suites check that every staged case validates against `case.schema.json` and every one of its lines passes the line schema. No runner executes them, and the generator has no reference `validate_next`: until the build, a staged reject case proves only that every line is admitted by the schema. Its rule is checked when the runners take it on; where an earlier event could plausibly break a rule first, the case has an accepted twin whose log differs only in the breaking event (`woken-principal-spawning-run`, `legacy-wakes-grouped-by-run`). The build moves each family from `_build_staged` to `FAMILIES` in `spec/tools/fixtures/__main__.py` in the PR that makes both runners pass it, and drops the pre-build refusal tests, so a staged case is never a skipped case.

**Planned with the build**, recorded here so the Phase 0 and Phase 1 proof lists are complete (Gate 1 §7): Phase 0's unknown-recipient refusal, handoff keeping its tool context, today's team parity and the Python run SSE bounded to its run; the legacy-wake schedules (two children at one boundary share one `woken`, a crash before the combined append, cancel after a late result, a parked child dropping its `pending_wakes` row, an index wipe); and Phase 1's two-process crash schedules, deadline and already-settled races, homogeneous batches, member parks, deletion, tree accounting and ancestry. The typed `agent()` overloads are type-tested in both languages with the build: `team` absent gives `Agent`, `team: []` and a non-empty team give `TeamAgent`, and `TeamAgent.run()` and `.stream().result` are `TeamRunResult` with `team`. They need a runtime or several processes, so they arrive as `team` cases or per-language tests with the build.
