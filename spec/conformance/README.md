...
`vectors/*.json` are small shared value vectors for rules that are not a whole case, such as manifest ordering (`vectors/manifest-order.json`). Each vector's expected value is checked by both test suites, and was computed independently by `spec/tools` and by the TypeScript implementation. Unlike cases, a vector is small and hand-written; change one only together with the rule it pins.
