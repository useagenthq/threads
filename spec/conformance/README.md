...
`staged/` holds cases for a spec whose Gate 2 is approved and whose build hasn't landed: today the Teams contract (`../schema/README.md`, "Teams", semantic rules 31-45). `gen_fixtures.py` writes and `--check`s them like the corpus, and both test suites check that every staged case validates against `case.schema.json` and every one of its lines passes the line schema. No runner executes them, and the generator has no reference `validate_next`: until the build, a staged reject case proves only that every line is admitted by the schema. Its rule is checked when the runners take it on; where an earlier event could plausibly break a rule first, the case has an accepted twin whose log differs only in the breaking event (`woken-principal-mail-run`, `run-final-answer-after-wake`). The build moves each family from `_build_staged` to `FAMILIES` in `spec/tools/fixtures/__main__.py` in the PR that makes both runners pass it, and drops the pre-build refusal tests, so a staged case is never a skipped case.
