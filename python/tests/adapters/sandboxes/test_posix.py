"""The sandbox-side scripts every adapter shares: the manifest order, and the exec wrapper's
argv carries env names, never values. (An export is read by the strict tree reader: its tests
are tests/sandbox/test_tar_*.py and test_trees.py.)"""
...
def test_the_manifest_matches_the_shared_vector() -> None:
    vector = json.loads(VECTOR.read_text(encoding="utf-8"))
    tree = {f["path"]: f["data"].encode() for f in vector["files"]}
    assert [e["path"] for e in manifest_of(tree)] == vector["manifest_paths"]
    assert manifest_hash(manifest_of(tree)) == vector["manifest_hash"]
