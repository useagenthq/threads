"""Runner for the `fork` conformance cases (spec/conformance/README.md, "What a runner does per
kind"): import the log, fork at the case's event against a sandbox, and compare the child's
lines, the fork facts, the ledger rows and the untouched parent. The fake sandbox and every
real adapter over its mocked backend run the same cases."""
...
def script_of(case: Path) -> dict[str, JsonValue]:
    return load(case, "sandbox.json") if "sandbox_script" in load(case, "case.json") else {}
...
async def run_case(case: Path, sandbox: Sandbox, creates: Callable[[], int]) -> Outcome:
    """Forks per the case against `sandbox`; `creates` counts its provider creates."""
...
async def _fork(
    store: SqliteStore, sandbox: Sandbox, at: ForkAt, now: int
...
def assert_expected(name: str, got: Outcome) -> None:
    """The case's expected.json holds for `got`."""
