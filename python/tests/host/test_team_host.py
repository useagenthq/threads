...
def test_a_schedules_new_thread_of_a_lead_opens_its_team() -> None:
    async def main() -> None:
        store = sqlite(":memory:")
        lead = agent(name="lead", model=scripted_model({"responses": []}), team=[])
        runner = Runner(store, {"lead": lead}, {})
        started = await pinned_start(lead.definition, store)
        due = [Due("daily", 60_000, "lead", "Report.", "UTC", missed=False)]
        assert await reserve_due(Pass(runner, store, "local"), started, due, 60_000) is None
        sq = await open_store(store)
        teams = await sq.run(lambda c: c.execute("SELECT lead_thread_id FROM teams").fetchall())
