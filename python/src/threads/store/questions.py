...
def record(conn: Conn, tenant_id: str, events: Sequence[StoredEvent], now: int) -> None:
    for event in events:
...
def open_row(conn: Conn, tenant_id: str, event: ParkedEvent) -> None:
    write_rows(conn, tenant_id, Rows((), (QuestionRow(event, "open"),)))


def settle(conn: Conn, event: ToolResultEvent, now: int) -> None:
