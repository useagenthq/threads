...
from threads.hooks.runner import RESULT, TEXTS, TOOL, decision_draft, injected
...
def _strictest(verdicts: set[str]) -> Verdict:
    """Deny absorbs ask absorbs allow; anything else (a failure) denies."""
    known: set[Verdict] = {_VERDICTS.get(v, "deny") for v in verdicts}
    if "deny" in known:
        return "deny"
    return "ask" if "ask" in known else "allow"


async def authorize(
    rt: Runtime, call: ToolCallEvent, policy: Decision
) -> tuple[Decision, list[Draft]]:
    """The call's decision with its hooks folded in. Deny absorbs everything: a policy deny
    runs no hook, and no hook can turn a deny or an ask into an allow except the
    permission_request approver, which only answers an ask."""
    ids = {"call_id": call.data.call_id}
    drafts: list[Draft] = []
    decision = policy
    if policy.decision != "deny" and rt.hooks.has("before_tool"):
        ran = await rt.hooks.run("before_tool", TOOL, call.data)
        drafts += [
            decision_draft("before_tool", r, verdict(r), said(r, "reason"), **ids) for r in ran
        ]
        hooked = _strictest({verdict(r) for r in ran})
        if _RANK[hooked] >= _RANK[policy.decision]:
            decision = Decision(hooked, "hook")
    if decision.decision == "ask" and rt.hooks.has("permission_request"):
        ...
        decision = Decision(_strictest({verdict(r) for r in ran}), "hook")
    if decision.decision == "ask" and rt.fold.mode == "dont_ask":
        decision = Decision("deny", decision.source, decision.rule)
...
async def _result_gate(rt: Runtime, result: ToolResultEvent) -> Gated:
    """redact appends context_edited{guardrail, redact}; deny (or a failure) clears the result
    from every later request. The raw result stays in the log."""
    call_id = result.data.call_id
    call = rt.fold.calls[call_id]
    ran = await rt.hooks.run("before_tool_result", RESULT, call.data, result.data)
    ids = {"call_id": call_id}
    drafts = [
        decision_draft("before_tool_result", r, verdict(r), said(r, "reason"), **ids) for r in ran
    ]
    spans: list[JsonValue] = []
    for r in ran:
        if r.value is not None and r.value["decision"] == "redact":
            spans += [to_json(span) for span in r.value["spans"]]
    edit: dict[str, JsonValue] | None = None
    if any(verdict(r) == "deny" for r in ran):
        edit = {"call_id": call_id, "action": "clear"}
    elif spans:
        edit = {"call_id": call_id, "action": "redact", "part": 0, "spans": spans}
    if edit is not None:
