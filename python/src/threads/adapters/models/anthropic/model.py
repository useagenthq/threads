...
class AnthropicModel:
    ...
    async def send(self, request: ModelRequest, context: ModelContext) -> AsyncIterator[ModelChunk]:
        ...
        async for chunk in _stream(response, assembler):
            yield chunk
...
async def _stream(response: object, assembler: Assembler) -> AsyncIterator[ModelChunk]:
    """A stream error before any content is a rejection; after content it is uncertainty."""
    if not isinstance(response, httpx2.Response):
        raise TypeError("the SDK returned no raw response")
    started = False
    try:
        async for sse in transport.sse(response):
            event = parse(sse.data)
            if event is None:
                continue
            try:
                chunks = await assembler.feed(event)
            except ProviderStreamError as error:
                rejected = _STREAM_ERRORS.get(error.kind)
                if started or rejected is None:
                    raise
                yield rejected
                return
            for chunk in chunks:
