...
|---|---|---|---|
| E2B | `@threads/e2b` (runs on Bun) | `threads.e2b` | [E2B](/sandboxes/e2b) |
| Daytona | `@threads/daytona` | `threads.daytona` | [Daytona](/sandboxes/daytona) |
| Modal | not available yet | `threads.modal` | [Modal](/sandboxes/modal) |

## What the agent gets

With a sandbox, the agent can use these [built-in tools](/agents/built-in-tools). Paths are relative to `/workspace`.
...
A sandbox also enables the git, code intelligence (`lsp`) and computer-use tools when you turn them on. See [Built-in tools](/agents/built-in-tools). Without a sandbox, none of these tools exist.
...
When you [fork](/evals/fork) a thread, the new branch restores the sandbox into a fresh, separate machine. Experiments on a fork never touch the original sandbox.
...
If the host crashes while a command is running, threads can't prove the command stopped, so it does not run it again on its own. The run parks and waits for you to decide. See [Durability](/production/durability).

## Testing

Use `fakeSandbox()` / `fake_sandbox()` in tests: an in-memory sandbox with scripted commands, no provider and no network. It also takes snapshots, so you can test forks. See [Scripted model and fake sandbox](/evals/testing).
