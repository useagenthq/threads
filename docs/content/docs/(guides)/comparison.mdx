...
threads has [subagents](/docs/multi-agent/subagents), [handoffs](/docs/multi-agent/handoffs) and [teams](/docs/multi-agent/teams), where a lead and its members share a task board and a mailbox. Messaging between agents outside a team and the A2A protocol are planned, not built. Strands (swarm, graph, A2A) and Pydantic AI (graphs, A2A) offer more patterns today.
