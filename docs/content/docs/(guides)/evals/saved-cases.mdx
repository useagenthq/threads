...
`thread` here is a handle opened with a sandbox, as in [Fork](/docs/evals/fork). `CaseExpectation` is imported from `threads` in Python; in TypeScript it is a plain object.
...
`portable` is `true` when the snapshot came from the in-memory [fake sandbox](/docs/evals/testing), so the case needs no sandbox provider to replay. It is `false` when it depends on a real provider such as E2B.
...
- replay the same turn yourself with a [scripted model](/docs/evals/testing) built from `model.json`;
- re-run the scenario on a [stub fork](/docs/evals/fork#replay-without-real-side-effects) (Python) and compare against `must`.
