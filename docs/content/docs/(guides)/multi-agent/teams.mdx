...
| `send_message` | lead and members | Sends `text` to a member by agent name, or to `*` for everyone. A name that isn't on the team (the lead or one of its subagents) is refused, and the error lists the names that are. |
| `spawn_agent` | lead | Starts a member. See [Subagents](/docs/multi-agent/subagents). |
...
- **The lead's thread holds the team.** Tasks, claims and messages are recorded in the lead's log, so you can see who did what in the lead's [timeline](/docs/evals/timeline).
...
- **Run members in parallel** by starting them with `background: true`. Each result wakes the lead for another turn, and the lead's run returns once all of them have reported, with the lead's last answer. See [Background subagents](/docs/multi-agent/subagents#background-subagents).
