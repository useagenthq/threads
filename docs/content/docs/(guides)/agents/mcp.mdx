...
<Field name="defer" type="boolean" default="false">
  The model sees only the server's tool names until it loads them with `tool_search`. Use it for servers with many tools. See [load them on demand](/docs/agents/tools#many-tools-load-them-on-demand).
</Field>
