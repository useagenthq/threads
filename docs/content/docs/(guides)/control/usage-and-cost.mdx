...
```python
# A price on the model is what lets cost() count money (see Models).
bot = agent(model=anthropic("claude-sonnet-5", price=Price(input=2000, output=10000)))
...
`cost()` prices the recorded usage with the model's `price`. An agent whose model has a price records its currency as USD, so the model factory's `price` is in nano-dollars per token (USD 2 per million tokens is `2000`, see [Models](/docs/agents/models)).
