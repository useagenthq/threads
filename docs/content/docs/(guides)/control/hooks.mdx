...
<Field name="tools" type="Tool[]">
  Tools this extension brings. They get the run's `deps`, like the agent's own tools.
</Field>
...
Every hook is optional, awaited and time-limited. Each gets the run context last (`ctx`: thread id, principal and the `deps` you passed to `run()`).

| TypeScript | Python | When | Returns |
|---|---|---|---|
| `sessionStart` | `session_start` | A run starts, resumes, or is the first run on a forked branch | Text to add as context |
...
| `notification` | `notification` | The run parked, scheduled a retry, ran out of budget, or a subagent finished | Nothing |

A `deny` always carries a `reason`, recorded in the log. When several extensions define `beforeTool`, the strictest answer wins, and the first `deny` stops the gate: later extensions aren't called. `redact` spans are UTF-8 byte offsets into the result's first text part (the preview when it has no content); an empty list, an empty span, or a span outside that text or splitting a character fails the hook and clears the whole result. For a denied tool call, the model sees `denied: <reason>` as the result. Text a hook adds is shown to the model as context from the hook.
