...
<Field name="channels" type="Record<string, ChannelAdapter>">
  ...
  A channel adapter you write yourself must also implement `renderText(text)` (`render_text` in Python): the ops for a plain message the host sends on its own, an `ask_user` question or the correction after a reply that isn't one of its options. The bundled adapters have it. Without it, TypeScript doesn't compile and Python's `ready()` fails with `invalid_config`.
</Field>
