...
| `stop()` | Cancels runs, starts nothing new, then waits for work in flight and releases the threads this process owns. A reply already on its way is finished first, so it is never sent twice. `stop()` has no timeout: put deadlines on your tools and providers. |
