...
<Steps>
  ...
  <Step>
    ...
    The response is a Server-Sent Events stream. Each logged step arrives as an `event` message with its sequence number as the SSE id, and the stream ends with the run's `result`. A run that started [background subagents](/docs/multi-agent/subagents#background-subagents) streams through each wake turn they cause, so the result comes after the last one:
...
| `POST`, `GET` | `/v1/ui/...` | Chat UIs over the AI SDK and AG-UI protocols. See [Frontends](/docs/host/frontends) |
