...
// A tool that needs approval ends the run with an interrupt. Answer it to resume.
for (const interrupt of agent.pendingInterrupts) {
  await agent.runAgent({
    resume: [{ interruptId: interrupt.id, status: "resolved", payload: { decision: "grant" } }],
  });
}
```

- `threadId` is your chat's key, scoped to the caller and the agent just like the AI SDK's `id`.
- An approval is answered with `{decision: "grant" | "deny"}`, and `status: "cancelled"` denies it. A question takes `{answer: "..."}`. Each interrupt carries its `responseSchema`.
- If someone else settled the interrupt first (an approver in Slack, say), resuming still works. The stream carries a `threads.resume_conflict` custom event that tells you what was recorded.
