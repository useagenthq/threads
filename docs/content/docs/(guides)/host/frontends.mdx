...
- When a tool needs [approval](/docs/control/human-in-the-loop), or the agent asks the user a question with `ask_user`, the user's answer is sent automatically and the run continues in the same assistant message. A free-text question has no `options`: send the typed text as the `output`.
