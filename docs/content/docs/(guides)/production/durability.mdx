...
A thread is pinned to the agent config it started with. Running it with a different model, tools or instructions raises a `ConfigError` ("a config change starts a new thread"). A threads release can change what is pinned too; when it does, the error says so, and the [release notes](/docs/production/release-notes) list it.
...
| `sandbox_local` | Changes that stay inside the sandbox (the built-in `bash`, `write`, `edit` and `notebook_edit` when the sandbox has no internet; with `egress: "unenforced"` they are `unguarded`) | Once the sandbox process is confirmed gone, the model is told the call was interrupted and may have partly run. Otherwise parks |
