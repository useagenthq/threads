...
<Field name="defer" type={"boolean"} default={"false"}>
  true: the model sees only this tool's name, listed in tool_search's description, until it loads the tool with tool_search; its description and schema are sent only after the load. Can't be combined with ends_turn (setup error invalid_config). context.defer_tools decides whether the flag is honoured. Default false.
</Field>
