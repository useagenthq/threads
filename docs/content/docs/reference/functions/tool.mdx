...
<Field name="defer" type={"boolean"} default={"false"}>
  true: the model doesn't see this tool's schema until it finds it with tool_search; can't be combined with ends_turn (setup error invalid_config). The model sees the tool's name and description until then. context.defer_tools decides whether the flag is honoured. Default false.
</Field>
