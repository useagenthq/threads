...
<Field name="lifetimeMs / lifetime_ms" type={"number"} default={"3600000"}>
  How long a sandbox lives, in milliseconds, before E2B kills it (the declared expiry).
</Field>
...
**Throws** `ConfigError` with one of these codes: `missing_secret`, `transport_fence_unsupported` (TypeScript only).
