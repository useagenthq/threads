---
title: "secret"
description: "A reference to a host secret, resolved only in the host."
icon: "SquareFunction"
---

Values never reach the log, prompts or the sandbox. A resolved value must be at least 8 characters and not a literal of the event schema, or resolving it is ConfigError invalid_config.

Import from `@threads/core` (TypeScript) or `threads` (Python).

<Tabs items={["TypeScript", "Python"]} groupId="lang" persist>
<Tab value="TypeScript">

```ts
function secret(name: string): Secret
```

</Tab>
<Tab value="Python">

```python
def secret(
    name: str,
) -> Secret
```

</Tab>
</Tabs>

## Parameters

<Field name="name" type={"string"} required>
  The name of the host environment variable that holds the value. It is read only when host code needs it; an unset or empty variable is a setup error missing_secret.
</Field>

## Returns

[`Secret`](/docs/reference/types/Secret)
