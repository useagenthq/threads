...
Resolve the search key and check configuration on the host. Called by check() or the first run, before anything is pinned; never at import or construction. Retried on the next check() or run after a failure.
