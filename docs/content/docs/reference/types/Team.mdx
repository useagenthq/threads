...
<Field name="mode" type={"\"all\" | \"any\" | PosInt"} default={"all"}>
  all, any, or how many must settle. any never cancels the others. A number that is not a positive integer, or one above the count of distinct members, is refused invalid_request before anything is recorded.
</Field>
