...
<Field name="to" type={"MemberRef"} required>
  The member to ask. See [MemberRef](/docs/reference/types/MemberRef).
</Field>
...
<Field name="members" type={"readonly MemberRef[]"} required>
  The members to wait for: non-empty, frozen at the call.
</Field>

<Field name="mode" type={"\"all\" | \"any\" | PosInt"} default={"all"}>
  all, any, or how many must settle. any never cancels the others. A number above the count of distinct members is refused invalid_request before anything is recorded.
</Field>
