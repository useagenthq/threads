...
<Field name="writeEffect / write_effect" type={"EffectClass"}>
  The effect class of save_memory and forget_memory with this provider. Absent: unguarded. idempotent requires dedup_window_ms: the provider dedups on the key within it. Python declares it with dedup_window_ms (the DeclaresWrites protocol); a provider that declares neither is unguarded.
</Field>

<Field name="dedupWindowMs / dedup_window_ms" type={"PosInt"}>
  How long, in milliseconds, the provider dedups a write on its key. Required when write_effect is idempotent; otherwise leave it out (Python: None). local_memory keeps keys forever.
</Field>
