...
<Field name="request" type={"ModelRequest"} required>
  The request to count, as send would receive it. Neither language's loop calls count_tokens today, and the Python model protocol has no such method yet. See [ModelRequest](/docs/reference/types/ModelRequest).
</Field>
