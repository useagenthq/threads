...
thread is the Thread handle in both languages: read it (usage(), timeline(), fork()) or pass it back to run() to continue. The HTTP form is host-api RunOutcome (ids instead of handles).
