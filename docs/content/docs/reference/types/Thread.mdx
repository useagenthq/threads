...
Answers an open ask_user question, recorded as that call's result, then resumes the branch. No built-in tool asks questions yet.
...
<Field name="answer" type={"string | readonly string[]"} required>
  The answer text, or the chosen options as a list (joined with newlines).
</Field>
