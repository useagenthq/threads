...
| `GET` | `/v1/threads/{thread_id}/timeline` | The thread's steps. See [Timeline](/evals/timeline) |
| `GET` | `/v1/threads/{thread_id}/branches` | List branches |
| `GET` | `/v1/threads/{thread_id}/fork-points` | Where the thread can be forked |
| `POST` | `/v1/threads/{thread_id}/forks` | Fork a branch. See [Fork](/evals/fork) |
...
The full request and response schemas are in the [HTTP API reference](/reference/overview), generated from the OpenAPI file at `spec/schema/host-api/openapi.json`.
