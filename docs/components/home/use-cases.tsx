...
export function UseCases() {
  return (
    <Tabs defaultValue={USE_CASES[0]?.id} className="mt-12">
      <TabsList aria-label="Use cases" className="grid grid-cols-2 gap-2 lg:grid-cols-4">
        {USE_CASES.map((u) => (
          <TabsTrigger
            key={u.id}
            value={u.id}
            className="rounded-xl border border-fd-border bg-fd-background px-4 py-3 text-left text-sm font-medium text-fd-muted-foreground transition-colors hover:text-fd-foreground data-[active]:border-fd-primary/50 data-[active]:bg-fd-card data-[active]:text-fd-foreground"
          >
            {u.title}
          </TabsTrigger>
        ))}
