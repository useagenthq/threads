...
<ParamField path="input" type="schema" required>
  A Zod schema (TypeScript) or a Pydantic model class (Python). Unknown keys are rejected.
</ParamField>
<ParamField path="runs" type='"host"' required>
  Where the tool runs. Tools run in your process today, so this is always `"host"`.
</ParamField>
<ParamField path="execute" type="(input, ctx) => Promise<Output>" required>
  Your function. It gets the parsed input and a [run context](/reference/types/RunContext) with your `deps`. A string result is shown to the model as is; anything else as JSON.
</ParamField>
<ParamField path="effect" type="string" default="unguarded">
  What your tool does to the outside world. See below.
</ParamField>
<ParamField path="endsTurn" type="boolean" default="false">
  `ends_turn` in Python. When the call succeeds, the turn ends without another model call.
</ParamField>
<ParamField path="output" type="Zod schema">
...
`effect` also sets the default permission: in the default mode, `read_only` tools run freely and every other tool asks for approval first. See [Permissions](/control/permissions) to allow them.
...
Leave `effect` out when you can't check or dedupe. If a call is interrupted, threads parks the run instead of guessing, and you settle it by hand. See [Durability](/production/durability).
