...
Then run the thread again to continue. A host exposes the same action over HTTP, see [HTTP API & SSE](/host/http-api). Approvals park the same way; see [Human-in-the-loop](/control/human-in-the-loop).
...
- `modal()` sandboxes in TypeScript. Modal works in Python, see [Modal](/sandboxes/modal).
...
<Card title="Inspect a thread" icon="history" href="/evals/timeline">
