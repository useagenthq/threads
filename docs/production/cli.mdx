...
Packages are not published yet, so run the CLI from your clone. See [Installation](/installation).
...
Prints one JSON object per step of the main branch (or the given one): the logged event plus whether it is a place you can fork from. See [Timeline](/evals/timeline).
