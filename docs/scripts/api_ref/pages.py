...
from .members import annotate, errors_line, fields_section, lang_of, member_doc, param_fields
...
def function_page(api: Obj, key: str, f: Obj) -> str:
...
def method_section(type_name: str, name: str, m: Obj) -> str:
    ...
    if params:
        out += ["", param_fields(container, params)]
    extra = errors_line(m)
    if extra:
        out += ["", extra]
    return "\n".join(out)
...
def shape_section(name: str, t: Obj, casing: str) -> list[str]:
    ...
    if kind == "object":
        return ["## Fields", "", fields_section(name, obj(t["fields"]), casing)]
    if kind != "union":
        return []
    ...
    for variant in objs(t["variants"]):
        ...
        out += [f"### {tag}", "", fields_section(name, fields, variant_casing), ""]
    return out


def type_page(name: str, t: Obj) -> str:
    ...
    body += shape_section(name, t, casing)
    ...
    if methods:
        ...
        for key, m in methods.items():
            body += [method_section(name, key, obj(m)), ""]
    return "\n".join(body).rstrip() + "\n"
