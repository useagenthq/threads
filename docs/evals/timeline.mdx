...
<Note>
The `sandbox` option is only needed if you want to [fork](/evals/fork) or [save a case](/evals/saved-cases) from this handle.
</Note>
...
A thread starts with one branch. Every [fork](/evals/fork) adds another. `branches()` lists them:
...
`threads timeline <thread_id>` prints the same timeline without writing code. See [CLI](/production/cli).

<CardGroup cols={2}>
  <Card title="Fork a thread" icon="git-fork" href="/evals/fork">
    Branch from a snapshot and try something different.
  </Card>
  <Card title="Thread reference" icon="book-open" href="/reference/types/Thread">
