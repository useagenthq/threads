...
The script is the same format `saveCase` writes to `model.json`, so a [saved case](/evals/saved-cases) doubles as a script.
...
Add `is_error: true` to make the command exit with status 1. Any other command answers `command not found`. It snapshots like a real provider, so [fork](/evals/fork) and [saved cases](/evals/saved-cases) work in tests too, and it blocks all network access.
