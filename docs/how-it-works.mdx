...
<AccordionGroup>
  <Accordion title="Resume after a crash" icon="rotate-ccw">
    Run the same thread again and threads picks up where the log ends. A step that was recorded as finished is never redone. See [Durability](/production/durability).
  </Accordion>
  <Accordion title="No double actions" icon="shield">
  ...
  <Accordion title="One process at a time" icon="lock">
    Only one process can drive a thread at a time. A second one is refused with `branch_busy` instead of both running the agent.
  </Accordion>
  <Accordion title="Inspect and fork" icon="git-fork">
    [`timeline()`](/evals/timeline) shows every step with the exact request the model saw. [`fork()`](/evals/fork) starts a new branch from a past step, in its own sandbox, so you can try a different input or a fix without touching the original.
  </Accordion>
  <Accordion title="A stable prompt prefix" icon="layers">
  ...
  <Accordion title="One format for both languages" icon="languages">
...
| Built-in shell and file tools | The [sandbox](/sandboxes/overview), with no internet by default and no credentials |
| Channels, schedules, the HTTP API | The optional [host](/host/overview), started with `threads dev` or `threads start` |

## Learn more

<CardGroup cols={2}>
  <Card title="Durability & crash safety" icon="shield-check" href="/production/durability">
